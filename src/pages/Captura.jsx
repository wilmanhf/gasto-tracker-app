      ...
      <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-md p-8 max-w-sm w-full text-center">
          ...
          <p className="text-gray-600 mb-6">
            {datos.tipo === "gasto" ? "Gasto" : "Ingreso"} de <strong>${parseFloat(datos.monto).toFixed(2)}</strong> registrado.
          </p>
          <button onClick={reiniciar} className="w-full bg-orange-600 text-white rounded-lg py-3 font-semibold hover:bg-orange-700 transition">
            📸 Capturar otro
          </button>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gray-100 pb-10">
      ...
      <div className="px-4 -mt-2 max-w-lg mx-auto">
        {mensaje && (
          <div className={`mt-4 p-3 rounded-lg text-sm font-medium ${mensaje.tipo === "error" ? "bg-red-100 text-red-700" : "bg-green-100 text-green-700"}`}>
            {mensaje.texto}
          </div>
        )}
        {paso === "captura" && (
          <div className="mt-6 space-y-4">
            {imagen ? (
              <div className="relative">
                <img src={imagen} alt="Comprobante" className="w-full rounded-lg shadow object-contain max-h-64" />
            ...
            <div className="grid grid-cols-2 gap-3">
              <button onClick={() => camaraRef.current.click()} className="bg-gray-700 text-white rounded-lg py-4 font-semibold text-sm hover:bg-gray-800 transition flex flex-col items-center gap-1">
                <span className="text-2xl">📷</span>Cámara
              </button>
              <button onClick={() => fileRef.current.click()} className="bg-orange-600 text-white rounded-lg py-4 font-semibold text-sm hover:bg-orange-700 transition flex flex-col items-center gap-1">
            ...
            {imagen && (
              <button onClick={analizarConOCR} disabled={procesando} className="w-full bg-orange-600 text-white rounded-lg py-4 font-bold text-base hover:bg-orange-700 transition disabled:opacity-60">
                {procesando ? (
            ...
            <button onClick={() => setPaso("revision")} className="w-full border border-gray-300 text-gray-700 rounded-lg py-3 text-sm hover:bg-gray-50 transition">
        ...
        {paso === "revision" && (
          <div className="mt-6 bg-white rounded-lg shadow p-5 space-y-4">
            {imagen && <img src={imagen} alt="Comprobante" className="w-full rounded-lg object-contain max-h-40 mb-2" />}
            <div>
            ...
            <div className="flex gap-3 pt-2">
              <button onClick={() => setPaso("captura")} className="flex-1 border border-gray-300 text-gray-700 rounded-lg py-3 text-sm hover:bg-gray-50 transition">← Volver</button>
              <button onClick={guardar} disabled={guardando} className="flex-1 bg-orange-600 text-white rounded-lg py-3 font-bold text-sm hover:bg-orange-700 transition disabled:opacity-60">
